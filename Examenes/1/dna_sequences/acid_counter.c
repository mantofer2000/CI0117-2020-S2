...
void* process_dna(void* args){
    ...
    if(my_dna == 1){
        ...
        for(size_t i = my_thread_id%shared_data->thread_amount; i < sizeof(DNA_SEQUENCE_2) - 1; i = i + jump){
    ...
    for(size_t i = my_thread_id; i < ARRAY_SIZE; i = i + big_jump){
        ...
        pthread_rwlock_rdunlock(&shared_data->rwlock_dna_1[i]);
        pthread_rwlock_rdunlock(&shared_data->rwlock_dna_2[i]);
    }  
