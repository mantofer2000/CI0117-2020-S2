...
void initialize_fight(player_t * p_one, player_t * p_two){
    ...
    battle_arena->battle_over = is_battle_over(players_array);
...
int is_battle_over(player_t ** players_array){
    int is_over = (players_array[TEAM_1]->pokemon_availible && players_array[TEAM_2]->pokemon_availible) ? FALSE : TRUE;
    return is_over;
}